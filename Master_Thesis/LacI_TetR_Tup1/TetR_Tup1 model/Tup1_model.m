...
p1 = addparameter(TetR_Tup1_model, 'krnrTup1', 'Value', c = addcompartment(TetR_Tup1_model, 'comp'););
...
p11 = addparameter(TetR_Tup1_model, 'dTup1adh1', 'Value', 0.0077, 'ValueUnits', 'molarity');
p12 = addparameter(TetR_Tup1_model, 'KdTetR', 'Value', 0.5, 'ValueUnits', 'mole/liter');
...
raterule2 = addrule(TetR_Tup1_model, 'Citrimmature = k7tetCit * (kL7tet + (1 - kL7tet)/(1 + (Tup1free/ thetaTup1)^nTup1)) - (dCit + mu + kmaturation) * Citrimmature', 'RuleType', 'rate');
raterule3 = addrule(TetR_Tup1_model, 'Citrine = (kmaturation * Citrimmature) - (dCit + mu) * Citrine', 'RuleType', 'rate');

repassrule1 = addrule(TetR_Tup1_model, 'Tup1free = max(TetRTup1 - (TetR*TetRTup1 + TetRTup1*aTc_InUnit - TetRTup1*(KdTetRTup1_InUnit^2 + 2*KdTetRTup1_InUnit*TetR + 2*KdTetRTup1_InUnit*TetRTup1 + 2*KdTetRTup1_InUnit*aTc_InUnit + TetR^2 + 2*TetR*TetRTup1 - 2*TetR*aTc_InUnit + TetRTup1^2 - 2*TetRTup1*ATC_InUnit + aTc_InUnit^2)^(1/2) + TetRTup1^2 + KdTetRTup1_InUnit*TetRTup1)/(2*(TetR + TetRTup1)),0);', 'RuleType', 'repeatedAssignment');
