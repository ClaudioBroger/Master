...
void model(double time_local, double *stateVector, double *DDTvector, ParamData *paramdataPtr, int DOflag, double *variableVector, double *reactionVector, double *gout, int *eventVector)
{
    ...
    double eventassign_1_1,eventassign_2_1;
	...
	if (ge(TetR,1e-4)) {
		TetRfree = TetR/2.0-KdTetR_InUnit/2.0-atc/2.0+pow(pow(KdTetR_InUnit,2.0)+2.0*KdTetR_InUnit*TetR+2.0*KdTetR_InUnit*atc+pow(TetR,2.0)-2.0*TetR*atc+pow(atc,2.0),1.0/2.0)/2.0;
	} else {
		TetRfree = 0.0;
	}
    R1 = k7tetTetR*(kL7tet+(1.0-kL7tet)/(1.0+pow(TetRfree/thetaTetR,nTetR)));
    R2 = k7tetCit*(kL7tet+(1.0-kL7tet)/(1.0+pow(TetRfree/thetaTetR,nTetR)));
    R3 = kmaturation*Citimmature;
    D1 = (dTetR+mu)*TetR;
    D2 = (dCit+mu)*Citimmature;
    D3 = (dCit+mu)*Citrine;
    eventassign_1_1 = TetR;
    eventassign_2_1 = TetR;
    if (DOflag == DOFLAG_DDT) {
        ...
        gout[0] = ge(TetR,1e-4)-0.5;
        gout[1] = ge(time,indTime)-0.5;
...
void calc_ic_model(double *icVector, ParamData *paramdataPtr)
{
    ...
    atc = piecewiseIQM(3,atcAdded/nMperUnit,ge(time,indTime),0.0);;
    KdTetR_InUnit = KdTetR/nMperUnit;;
    TetRfree = piecewiseIQM(3,TetR/2.0-KdTetR_InUnit/2.0-atc/2.0+pow(pow(KdTetR_InUnit,2.0)+2.0*KdTetR_InUnit*TetR+2.0*KdTetR_InUnit*atc+pow(TetR,2.0)-2.0*TetR*atc+pow(atc,2.0),1.0/2.0)/2.0,ge(TetR,1e-4),0.0);
    TetR = 0.0;
    Citimmature = 0.0;
    Citrine = 0.0;
    icVector[0] = TetR;
